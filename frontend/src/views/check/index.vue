<template>
  <div class="dashboard-editor-container">
    <el-table
      ...
      <el-table-column :label="$t('table.time')" width="110px" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column :label="$t('table.limit')" width="110px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.choosed }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('table.startTime')" width="110px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.startTime }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('table.endTime')" width="110px" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column :label="$t('table.actions')" align="center" width="180" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <div v-if="scope.row.status === 'CHECKED'">
            <el-button type="success" size="mini" @click="start(scope.row)">{{ $t('table.start') }}</el-button>
          </div>
          <div v-if="scope.row.status === 'RUNNING'">
            <el-button type="warning" size="mini" @click="end(scope.row)">{{ $t('table.end') }}</el-button>
          </div>
          <div v-if="scope.row.status === 'DRAFT'">
...
import { fetchReleaseList, check, startClass, endClass } from '@/api/course'
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchReleaseList(this.listQuery).then(response => {
    ...
    approve(row) {
      check(row.id, 'CHECKED').then(response => {
        this.$notify({
          title: '成功',
          message: '已通过',
          type: 'success',
    ...
    reject(row) {
      check(row.id, 'REJECTED').then(response => {
        this.$notify({
          title: '成功',
          message: '已拒绝',
          type: 'success',
    ...
    start(row) {
      startClass(row.id).then(response => {
        this.$notify({
          title: '成功',
          message: '已开课',
          type: 'success',
